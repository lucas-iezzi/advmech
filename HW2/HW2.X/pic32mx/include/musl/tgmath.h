...
#define __tg_cmplx__(x) ((x)+(char complex)I)

/* The following definitions will work with bitfields, 
   yielding the integer-promoted type/size. */
#define __tg_safe_sizeof__(x) (sizeof(((x),(x))))
#define __tg_safe_typeof__(x) __typeof__(((x),(x)))

#define __IS_FP(x) (sizeof((x)+1ULL) == sizeof((x)+1.0f))

#define __IS_CX(x)   (__tg_safe_sizeof__(x) == sizeof(__tg_cmplx__(x)))
#define __IS_REAL(x) (2*__tg_safe_sizeof__(x) == sizeof(__tg_cmplx__(x)))

#define __FLT(x) (__IS_REAL(x) && __IS_FP(x) && __tg_safe_sizeof__(x) == sizeof(float))
#define __LDBL(x) (__IS_REAL(x) && __IS_FP(x) && __tg_safe_sizeof__(x) == sizeof(long double) && sizeof(long double) != sizeof(double))

#define __FLTCX(x) (__IS_CX(x) && __IS_FP(x) && __tg_safe_sizeof__(x) == sizeof(float complex))
#define __DBLCX(x) (__IS_CX(x) && (!__IS_FP(x) || __tg_safe_sizeof__(x) == sizeof(double complex)))
#define __LDBLCX(x) (__IS_CX(x) && __IS_FP(x) && __tg_safe_sizeof__(x) == sizeof(long double complex) && sizeof(long double) != sizeof(double))
...
*/
/* remove volatile from type of (non-void) x */
#define __nvtype(t) __typeof__(*((t*)0)+*((t*)0))
/* if c then t else void */
#define __typep(c,t) (0?(__nvtype(t)*)0:(void*)!(c))
#define __type1(c,t) __typeof__(*(__typep(c,t)))
/* if c then t1 else t2 */
#define __type2(c,t1,t2) __typeof__(*(0?(__typeof__(__typep(c,t1)))0:(__typeof__(__typep(!(c),t2)))0))
/* cast to double when x is integral, otherwise use typeof(x).
   if x is integral complex, case to double complex. */
#define __RETCAST(x) ( \
	__type2(__IS_FP(x), __tg_safe_typeof__(x), \
                __type2(__IS_CX(x), double complex, double)))
...
/* TODO: should work for sizeof(long double) == sizeof(double), but requires
   that __typeof__(__real__(x)) is allowed. */
#define __RETCAST_REAL(x) ( \
        __type2(!__IS_FP(x), double, \
	__type2(__IS_CX(x), __typeof__(__real__((__tg_safe_typeof__(x))x)), \
                __tg_safe_typeof__(x))))
/* add complex to the type of x */
#define __RETCAST_CX(x) (__typeof__(__RETCAST(x)0+(char complex)I))
#else
#define __RETCAST(x)
#define __RETCAST_2(x, y)
#define __RETCAST_3(x, y, z)
